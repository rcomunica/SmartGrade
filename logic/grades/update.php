...
$current_grade = getUserGradeById($pdo, $user_id, (int) $id);
if (!$current_grade) {
...
if (empty($errors) && isset($percentage)) {
    $current_total = getGradePercentageTotal($pdo, $user_id, (int) $subjet_id, (int) $term_id, (int) $id);
...
if (!empty($errors)) {
    ...
    header('Location: ../../views/grades/edit.php?id=' . (int) $id);
    exit;
}

$stmt = $pdo->prepare('UPDATE grades SET subjet_id = :subjet_id, term_id = :term_id, name = :name, value = :value, percentage = :percentage WHERE id = :id');
$stmt->execute([
