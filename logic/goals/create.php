...
$subject = getUserGoalSubjects($pdo, $user_id);
$subject_ids = array_column($subject, 'id');
if (!in_array((int) $subject_id, array_map('intval', $subject_ids), true)) {
...
$stmt = $pdo->prepare(
    'INSERT INTO goals (user_id, subject_id, name, description, target_date, status)
     VALUES (:user_id, :subject_id, :name, :description, :target_date, :status)'
);
$stmt->execute([
    'user_id' => $user_id,
...
$_SESSION['goals_success'] = 'Meta agregada correctamente.';
header('Location: ../../views/goals/index.php');
exit;
